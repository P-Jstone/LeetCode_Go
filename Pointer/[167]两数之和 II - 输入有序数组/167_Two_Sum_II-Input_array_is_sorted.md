...
<div><div>Related Topics</div><div><li>数组</li><li>双指针</li><li>二分查找</li></div></div><br><div></div>

#题解

由于数组是非递减顺序排序，所以在数组首尾各设置一个指针，计算数组两个指针处值的和，此时有以下三种情况：
1. 和大于target，此时需要减少总和，将右边的指针左移才能减小总和；
2. 和小于target，此时需要增加总和，将左边的指针右移才能增大总和；
3. 和等于target，此时将两个指针的位置加一就是所需答案