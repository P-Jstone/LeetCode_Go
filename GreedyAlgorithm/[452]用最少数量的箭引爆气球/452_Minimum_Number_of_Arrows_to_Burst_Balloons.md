...
<div><div>Related Topics</div><div><li>贪心</li><li>数组</li><li>排序</li></div></div><br><div></div>

#题解

本题实际上就是求重叠区间问题，几个区间有共同的重叠部分，那么这几个区间可以算作一组，尽可能多地将不同区间合并在同一组中，那么最终的组数就是最小弓箭数
可以将points数组根据 <code>x<sub>end</sub></code> 按照非递减顺序排序，然后从第二个元素遍历points数组，如果区间的首要小于等于之前重叠区间的尾，那么算作同一组，否则新加一组，并将当前比较的重叠区间末尾设为新的区间末尾